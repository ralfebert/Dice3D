...
class JAQDiceView: SCNView {
    ...
    @IBInspectable var maximumJumpHeight: CGFloat = 0.0
    @IBInspectable var squareSizeHeight: CGFloat = 0.0
    @IBInspectable var cameraPerspective = false
    var floorImage: UIImage = UIImage(named: "woodTile")!
    ...
    override func awakeFromNib() {
        ...
        if self.maximumJumpHeight == 0.0 {
            self.maximumJumpHeight = 120
        }
        if self.squareSizeHeight == 0.0 || self.squareSizeHeight < 30 {
            self.squareSizeHeight = 60
        }
    ...
    func loadScene() {
        ...
        self.camera?.position = SCNVector3Make(0, Float(self.maximumJumpHeight - 20), 0)
    ...
    @objc func checkStatus(_: Any?) {
        ...
        if ((result ?? 0.0) > 0.95 && (result ?? 0.0) < 1.05) || ((result ?? 0.0) < 0.05 && (result ?? 0.0) > -0.05) || ((result ?? 0.0) > -1.05 && (result ?? 0.0) < -0.95) {
            ...
            if self.timesStopped > threshold {
                self.timer?.invalidate()
                self.diceDelegate?.diceView(self, rolledWithFirstValue: self.dice1?.jaq_boxUpIndex() ?? 0, secondValue: self.dice2?.jaq_boxUpIndex() ?? 0)
            }
        } else {
            self.timesStopped = 0
        }
